...
template<typename x, bool realAdd, typename tuple>
struct Tuple_PushFront
{};
template<typename x, typename... tupleElems>
struct Tuple_PushFront<x, true, tuple<tupleElems...>>
{
  typedef tuple<x, tupleElems...> type;
};
template<typename x, typename... tupleElems>
struct Tuple_PushFront<x, false, tuple<tupleElems...>>
{
...
template<typename Pred, typename Head, typename... Ts>
struct Filter<Pred, tuple<Head, Ts...>>
{
  typedef
    typename Tuple_PushFront<Head,
                             Pred::template Apply<Head>::value,
                             typename Filter<Pred, tuple<Ts...>>::type>::type
      type;
};
...
struct ConcatenateTuple<Tuple0>
{
  typedef Tuple0 type;
};
...
struct ConcatenateTuple<Tuple0, Tuple1>
{
  ...
  struct ConcatenateImpl<tuple<TAs...>>
  {
    template<typename TupleB>
    struct Apply;
    template<typename... TBs>
    struct Apply<tuple<TBs...>>
    {
...
template<typename FirstTuple, typename... Follows>
struct ConcatenateTuple<FirstTuple, Follows...>
{
  typedef
    typename ConcatenateTuple<FirstTuple,
                              typename ConcatenateTuple<Follows...>::type>::type
      type;
};
...
struct QuickSort<tuple<T0>>
{
  typedef tuple<T0> type;
};
...
struct QuickSort<tuple<Head, Ts...>>
{
  ...
  typedef
    typename ConcatenateTuple<typename QuickSort<LeftElems>::type,
                              tuple<Head>,
                              typename QuickSort<RightElems>::type>::type type;
};
...
StaticTest()
{
  ...
  static_assert(
    is_same<Tuple_PushFront<i1, true, tuple<i2>>::type, tuple<i1, i2>>::value,
    "");
  static_assert(
    is_same<Tuple_PushFront<i2, true, tuple<i1>>::type, tuple<i2, i1>>::value,
    "");
  static_assert(
    is_same<Tuple_PushFront<i2, false, tuple<i1>>::type, tuple<i1>>::value, "");
